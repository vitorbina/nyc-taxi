"""
## DAG Documentation
# This DAG is responsible for automating the monthly extraction of New York City taxi trip data and loading it into our Data Lake (MinIO).
"""
from datetime import datetime
from airflow.decorators import dag, task
import logging
from utils.default import get_default_args
from utils.nyc import download_yellow_tripdata, upload_yellow_tripdata

# Logging configuration
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)


@dag(
    dag_id='nyc_taxi_ingestion',
    description='Pipeline Ingestion NYC Taxi (TaskFlow)',
    schedule='@monthly',
    start_date=datetime(2024, 1, 1),
    catchup=False,
    default_args=get_default_args()
)

def ingestion_pipeline():

    @task
    def download_taxi_nyc(logical_date):
        year = str(logical_date.year)
        month = f"{logical_date.month:02d}"
        logger.info(f"Processing, in {year}, Month: {month}")
        return download_yellow_tripdata(year, month)
    
    @task
    def upload_taxi_nyc(file_path, logical_date):
        year = str(logical_date.year)
        month = f"{logical_date.month:02d}"
        bucket_name = "data-lake-nyc"

        upload_yellow_tripdata(file_path, year, month, bucket_name)

    file_path = download_taxi_nyc()

    upload_taxi_nyc(file_path)

pipeline = ingestion_pipeline()
