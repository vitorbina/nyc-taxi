...
Final (MinIO) ── pre-aggregated tables optimized for BI queries
  ↓
Hive Metastore ── catalog for raw, staging and final (nyc_taxi_raw, nyc_taxi, nyc_taxi_final)
  ↓
...
On first access to `localhost:3000`, Metabase will ask you to set up an admin account and add a database connection.

Add a **Presto** connection (Metabase uses Presto driver for Trino):
...
| Port | 8080 |
| Catalog | hive |
| Schema | nyc_taxi_final *(or nyc_taxi / nyc_taxi_raw)* |

One Trino connection gives access to all three Hive catalogs (`nyc_taxi_raw`, `nyc_taxi`, `nyc_taxi_final`). Switch schema in Metabase per question.
